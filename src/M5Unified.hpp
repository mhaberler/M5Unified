...
class M5Unified {
   public:
    struct config_t {
        ...
        /// If auto-detection fails, the board will operate as the board
        /// configured here.
        ...
        union {
            ...
            [[deprecated("Change to external_speaker")]] struct {
                uint8_t enabled : 1;
    ...
    */
    ...
    // Find a display that matches the specified display type and designate it
    // as PrimaryDisplay. Multiple display types can be specified in the
    // initializer list.
    bool setPrimaryDisplayType(
        std::initializer_list<m5gfx::board_t> board_list);

    // Find a display that matches the specified display type and designate it
    // as PrimaryDisplay.
    bool setPrimaryDisplayType(m5gfx::board_t board) {
        return setPrimaryDisplayType({board});
    };
    ...
    void begin(void) {
        if (_board != m5gfx::board_t::board_unknown) {
            return;
        }
        config_t cfg;
        begin(cfg);
    ...
    defined(CONFIG_IDF_TARGET_ESP32S3)
        if (cfg.external_display.atom_display) {
            if (_board == board_t::board_M5Atom ||
                _board == board_t::board_M5AtomPsram ||
                _board == board_t::board_M5AtomS3 ||
                _board == board_t::board_M5AtomS3Lite) {
        ...
        // Module Display / Unit OLED / Unit LCD is determined after _begin
        // (because it must be after external power supply)
#if defined(__M5GFX_M5MODULEDISPLAY__)
#if !defined(CONFIG_IDF_TARGET) || defined(CONFIG_IDF_TARGET_ESP32) || \
    defined(CONFIG_IDF_TARGET_ESP32S3)
        if (cfg.external_display.module_display) {
            if (_board == board_t::board_M5Stack ||
                ...
                _board == board_t::board_M5StackCoreS3) {
        ...
        // Speaker selection is performed after the Module Display has been
        // determined.
        _begin_spk(cfg);
        ...
        if (cfg.external_display_value) {
            ...
            if (cfg.external_display.unit_oled) {
                if (cfg.unit_oled.pin_sda >= GPIO_NUM_MAX) {
                    cfg.unit_oled.pin_sda = (uint8_t)Ex_I2C.getSDA();
                }
                if (cfg.unit_oled.pin_scl >= GPIO_NUM_MAX) {
                    cfg.unit_oled.pin_scl = (uint8_t)Ex_I2C.getSCL();
                }
                if (cfg.unit_oled.i2c_port < 0) {
                    cfg.unit_oled.i2c_port = (int8_t)Ex_I2C.getPort();
                }
            ...
            if (cfg.external_display.unit_glass) {
                if (cfg.unit_glass.pin_sda >= GPIO_NUM_MAX) {
                    cfg.unit_glass.pin_sda = (uint8_t)Ex_I2C.getSDA();
                }
                if (cfg.unit_glass.pin_scl >= GPIO_NUM_MAX) {
                    cfg.unit_glass.pin_scl = (uint8_t)Ex_I2C.getSCL();
                }
                if (cfg.unit_glass.i2c_port < 0) {
                    cfg.unit_glass.i2c_port = (int8_t)Ex_I2C.getPort();
                }
            ...
            if (cfg.external_display.unit_lcd) {
                if (cfg.unit_lcd.pin_sda >= GPIO_NUM_MAX) {
                    cfg.unit_lcd.pin_sda = (uint8_t)Ex_I2C.getSDA();
                }
                if (cfg.unit_lcd.pin_scl >= GPIO_NUM_MAX) {
                    cfg.unit_lcd.pin_scl = (uint8_t)Ex_I2C.getSCL();
                }
                if (cfg.unit_lcd.i2c_port < 0) {
                    cfg.unit_lcd.i2c_port = (int8_t)Ex_I2C.getPort();
                }
...
#if defined(__M5GFX_M5MODULERCA__) || defined(__M5GFX_M5UNITRCA__)
            {
                ...
                if (unit_rca) {
                    if (board == board_t::board_M5Stack ||
                        ...
                        (!port_a_used &&
                         (  // ATOM does not allow video output via UnitRCA when
                            // PortA is used.
